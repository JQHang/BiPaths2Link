...
def fill_null_vectors(spark_session, df, column_name, vector_length = None):
    """
    Fills null vector column in a DataFrame with an empty SparseVector of a specified or calculated length.

    :param spark_session: SparkSession instance.
    :param df: DataFrame containing vector columns.
    :param column_name: Vector column name to check and fill.
    ...
    if vector_length is None:
        ...
        sample_vector = df.select(column_name).dropna().limit(1).collect()
        vector_length = len(sample_vector[0]) if sample_vector else 0
    ...
    df = df.withColumn(column_name, fill_vector_udf(col(column_name)))
