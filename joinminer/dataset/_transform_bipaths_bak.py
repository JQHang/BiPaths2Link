...
def _transform_bipaths(dataset_config, batch_df):
    ...
    for pair_node_type in ["head_node", "tail_node"]:
        ...
        if pair_node_type == "head_node":
            path_to_dir_type_list.append("forward_paths")
        else:
            path_to_dir_type_list.append("backward_paths")
    ...
    for path_dir_type in ["forward_paths", "backward_paths", "bipaths"]:
        ...
        for path_name in dataset_config[path_dir_type]:
            ...
            # 记录最终的path_to_feat_index, path_to_token_type, path_dir_type
            path_to_feat_index_list.append(path_to_feat_index)
            path_to_token_type_list.append(path_to_token_type)
            path_to_dir_type_list.append(path_dir_type)
    ...
    #########################################################################################
    # 获得三种类型的路径各自对应的最大总路径数
    path_dir_max_total_paths = []
    for path_dir_type in ["forward_paths", "backward_paths", "bipaths"]:
        path_dir_col_index = [i for i, s in enumerate(path_to_dir_type_list) if s == path_dir_type]
        
        total_paths_per_sample = np.sum(sample_to_path_count[:, path_dir_col_index], axis=1)
        path_dir_max_total_paths.append(np.max(total_paths_per_sample))
        
    max_total_paths = np.max(np.array(path_dir_max_total_paths))
    
    # 一次性分配sample_to_path_index矩阵
    sample_to_path_index = np.zeros((sample_count, 3, 1 + max_total_paths), dtype=int)

    # 设定三种路径各自对应的维度号，并添加cls位,记录各个样本当前填充到的位置
    path_dir_to_dir_index = {}
    path_dir_to_start_col = {}
    for path_dir_index, path_dir_type in enumerate(["forward_paths", "backward_paths", "bipaths"]):
        path_dir_to_dir_index[path_dir_type] = path_dir_index
        
        sample_to_path_index[:, path_dir_index, 0] = path_dir_index + 1
    
        # 记录每个样本当前填充到的列位置,也就是都填充了一个cls位
        path_dir_to_start_col[path_dir_type] = sample_count_ones

    # 创建列位置矩阵：(1, max_total_paths)
    col_positions = np.arange(1 + max_total_paths)[np.newaxis, :]
    
    # 累计path计数器，累积了一个pad,3种cls
    sample_path_cum_count = 4
    
    # 按照路径类型的顺序填充sample_to_path_index
    for col_idx in range(sample_to_path_count.shape[1]):
        # 获得该路径的方向
        path_dir_type = path_to_dir_type_list[col_idx]

        # 获得对应的dir_index
        path_dir_index = path_dir_to_dir_index[path_dir_type]
        
        # 当前路径类型各样本的path数量
        current_sample_path_count = sample_to_path_count[:, col_idx]

        # 获得各个样本对应的path的终止列
        sample_path_start_col = path_dir_to_start_col[path_dir_type]
        ...
        valid_positions = np.zeros((sample_count, 3, 1 + max_total_paths), dtype=bool)
        valid_positions[:, path_dir_index, :] = (col_positions >= sample_path_start_col) & (col_positions < sample_path_end_col)
        ...
        # 更新path对应的起始列
        path_dir_to_start_col[path_dir_type] = sample_path_end_col
