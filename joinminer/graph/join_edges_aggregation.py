...
@time_costing
def join_edges_table_token_aggregation(spark, join_edges_df, token_table_schema, agg_token_config, logger = setup_logger()):
    ...
    # 向量化及缩放
    # 注意，尝试过先缩放为vector再进行vector聚合运算，聚合结果不知为何丢失size参数，导致再合并多个聚合结果会触发first运算
...
@time_costing
def join_edges_aggregation(spark, graph, join_edges, target_nodes_config = None, logger = setup_logger()):
    # 可以基于是否给出inst_agg_path来决定是否保留计算结果，待优化
    logger.info(f"Aggregating instances for join-edges {join_edges['name']}")
    logger.info(f"The reulst will be output to: {join_edges['inst_agg_path']}")
    ...
    for seq_token_index, seq_token in enumerate(join_edges["flatten_format"]["seq"]):
        ...
        if "edge_type" in seq_token:
            ...
            for edge_table_name in [seq_token["edge_table_name"]]:
                ...
                # # 目前的写法，edge_limit和feat_cols的配合有问题（必须alias同一个名称才能limit，待优化）
