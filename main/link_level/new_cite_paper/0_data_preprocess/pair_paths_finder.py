...
join_edges_config_file = PROJECT_ROOT + '/main/config/join_edges_types/new_citation_relevant_paths.json'
...
from joinminer.graph import edge_to_intra_paths
...
edge_intra_paths = spark_runner.run(edge_to_intra_paths, graph, labeled_samples_config, join_edge_types, 
...
# 获得relevant_paths
relevant_paths = []
for hop_k in edge_intra_paths:
    for tgt_path in edge_intra_paths[hop_k]:
...
for relevant_path in sorted_relevant_paths:
    logger.info(f"Relevant path {relevant_path['name']} cover {relevant_path['match_metrics']['matched_distinct_edge_count']} "
